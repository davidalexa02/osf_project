<!-- cookie   -->

<footer class="text-center text-lg-start bg-white text-muted">

    <!-- Section: Links  -->
    <section class="">
        <div class="container text-start py-5">
            <!-- Grid row -->
            <div class="row mt-3">
                <!-- Grid column -->
                <div class="col-md-3 col-lg col-xl-3 mx-auto mb-4">
                    <!-- Content -->
                    <div class="text-left pb-4">
                        © Copyright <span id="year"></span><br>
                        <a class="text-reset" href="">All rights Reserved.</a>
                    </div>
                    <h6 class="text-uppercase fw-bold mb-4 mobdrop" onclick="showMenuF1()">
                        </i>Contact
                    </h6>
                    <div id="submenuf1" class="none">
                        <p>
                            Headquarters:
                            5600, Blvd. des Galeries, Bur 530 Québec, Québec G2K 2H6
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">contact@osf-global.com</a>
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">+1 (888) 548-4344</a>
                        </p>
                    </div>
                </div>
                <!-- Grid column -->

                <!-- Grid column -->
                <div class="col-md col-lg col-xl mx-auto mb-4">
                    <!-- Links -->
                    <h6 class="text-uppercase fw-bold mb-4 mobdrop" onclick="showMenuF2()">
                        Categoriest
                    </h6>
                    <div id="submenuf2" class="none">
                        <p>
                            <a href="error.php" class="text-reset">Alcohol</a>
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">Art</a>
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">Books</a>
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">Drink</a>
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">Electronics</a>
                        </p>
                    </div>
                </div>
                <!-- Grid column -->

                <!-- Grid column -->
                <div id="submenuf3" class="col-md col-lg col-xl mx-auto mt-5 mb-4 none">
                    <!-- Links -->
                    <p>
                        <a href="error.php" class="text-reset">Home</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Jewelry</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Kids & Baby</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Men's Fashion</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Mobile</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Motorcycles</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Movies</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Music</a>
                    </p>
                </div>
                <div id="submenuf4" class="col-md col-lg col-xl mx-auto mt-5 mb-4 none">
                    <!-- Links -->
                    <p>
                        <a href="error.php" class="text-reset">Sport</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Toys</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Travel</a>
                    </p>
                    <p>
                        <a href="error.php" class="text-reset">Women's Fashion
                        </a>
                    </p>
                </div>
                <div class="col-md col-lg col-xl mx-auto mb-4">
                    <!-- Links -->
                    <h6 class="text-uppercase fw-bold mb-4 mobdrop" onclick="showMenuF3()">
                        About
                    </h6>
                    <div id="submenuf5" class="none">
                        <p>
                            <a href="error.php" class="text-reset">About us</a>
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">Delivery</a>
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">Testimonials</a>
                        </p>
                        <p>
                            <a href="error.php" class="text-reset">Contact</a>
                        </p>
                    </div>
                </div>
                <!-- Grid column -->

                <!-- Grid column -->
                <div class="col-md col-lg col-xl-2 mx-auto mb-md-0 mb-4">
                    <!-- Links -->

                    <div class="d-flex justify-content-md-end justify-content-center text-muted">
                        <a href="" class="mx-md-auto mx-3 text-reset">
                            <i class="fab fa-facebook-f"></i>
                        </a>
                        <a href="" class="mx-md-auto mx-3 text-reset">
                            <i class="fab fa-twitter"></i>
                        </a>
                        <a href="" class="mx-md-auto mx-3 text-reset">
                            <i class="fab fa-google"></i>
                        </a>
                        <a href="" class="mx-md-auto mx-3 text-reset">
                            <i class="fab fa-instagram"></i>
                        </a>
                    </div>
                </div>
                <!-- Grid column -->
            </div>
            <!-- Grid row -->
        </div>
    </section>
    <!-- Section: Links  -->
</footer>





<div id="cookie-container" class="container py-3" aria-hidden="true">
    <div class="row">
        <div class="col-md-9">
            <h6 class="text-green">This website uses cookies</h6>
            <p>
                We use cookies in this website to give you the best experience on our
                site and show you relevant ads. To find out more, read our
                <a href="#">privacy policy</a> and <a href="#">cookie policy</a>.
            </p>
        </div>
        <div class="col-md-3 d-flex flex-column">
            <div id="closecookie" class="d-flex justify-content-end py-3">
                <i class="fa-solid fa-xmark"></i>
            </div>
            <button id="cookie-btn" class="text-uppercase">
                ACCEPT
            </button>
        </div>
    </div>
</div>